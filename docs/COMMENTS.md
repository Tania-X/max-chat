...
A comment must say something the code cannot: a constraint, a failure mode, an external contract, or a rejected alternative.
...
- Change narration — `# fixed X`, `# used to be Y`. That belongs in the commit message.
